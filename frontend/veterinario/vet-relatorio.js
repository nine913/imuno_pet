const usuarioString = localStorage.getItem('usuarioImunoPet');
if (!usuarioString) {
    window.location.href = 'index.html';
} else {
    const usuario = JSON.parse(usuarioString);
    if (usuario.perfil !== 'VETERINARIO') {
        window.location.href = 'dashboard.html';
    }
}

async function gerarRelatorio() {
    const dataInicio = document.getElementById('data_inicio').value;
    const dataFim = document.getElementById('data_fim').value;
    
    let url = 'http://localhost:3000/relatorio-vacinas?';
    if (dataInicio) url += `inicio=${dataInicio}&`;
    if (dataFim) url += `fim=${dataFim}`;

    const corpoTabela = document.getElementById('corpoTabela');
    const displayTotal = document.getElementById('totalGeral');

    try {
        const resposta = await fetch(url);
        const dados = await resposta.json();

        corpoTabela.innerHTML = '';

        if (dados.length === 0) {
            corpoTabela.innerHTML = '<tr><td colspan="5" style="text-align: center;">Nenhuma vacina aplicada neste período.</td></tr>';
            displayTotal.textContent = '0';
            return;
        }

        displayTotal.textContent = dados.length;

        dados.forEach(item => {
            const dataApp = item.data_aplicacao ? new Date(item.data_aplicacao).toLocaleDateString('pt-BR', { timeZone: 'UTC' }) : 'N/A';
            
            const tr = document.createElement('tr');
            tr.innerHTML = `
                <td>${dataApp}</td>
                <td><strong>${item.nome_vacina}</strong></td>
                <td>${item.nome_animal}</td>
                <td>${item.especie}</td>
                <td>${item.nome_tutor}</td>
            `;
            corpoTabela.appendChild(tr);
        });

    } catch (erro) {
        corpoTabela.innerHTML = '<tr><td colspan="5" style="text-align: center; color: red;">Erro ao gerar o relatório.</td></tr>';
    }
}

document.getElementById('btnGerarRelatorio').addEventListener('click', gerarRelatorio);

window.addEventListener('DOMContentLoaded', gerarRelatorio);