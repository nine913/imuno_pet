      ...
      <div style={{ padding: '40px', maxWidth: '1000px', margin: '0 auto', color: textColor }}>
        ...
        <div style={{ display: 'flex', gap: '20px', flexWrap: 'wrap' }}>
          ...
          <div style={{ flex: 1, minWidth: '300px', backgroundColor: bgCard, padding: '20px', borderRadius: '8px', border: `1px solid ${borderColor}` }}>
            <h3 style={{ color: headerColor, marginTop: 0 }}>Adicionar Espécie</h3>
            <form onSubmit={cadastrarEspecie} style={{ display: 'flex', gap: '10px' }}>
              <input 
                ...
                required 
                style={{ flex: 1, padding: '10px', border: `1px solid ${borderColor}`, borderRadius: '4px', backgroundColor: inputBg, color: textColor, fontSize: 'inherit' }} 
              />
            ...
            <ul style={{ listStyle: 'none', padding: 0, marginTop: '20px' }}>
              {especies.map(e => (
                <li key={e.id_especie} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px', borderBottom: `1px solid ${borderColor}` }}>
                  <span style={{ color: textSecundario, fontWeight: 'bold' }}>{e.nome_especie}</span>
          ...
          <div style={{ flex: 1, minWidth: '300px', backgroundColor: bgCard, padding: '20px', borderRadius: '8px', border: `1px solid ${borderColor}` }}>
            <h3 style={{ color: headerColor, marginTop: 0 }}>Adicionar Raça</h3>
            <form onSubmit={cadastrarRaca} style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              ...
              <div style={{ display: 'flex', gap: '10px' }}>
                <input 
                  ...
                  required 
                  style={{ flex: 1, padding: '10px', border: `1px solid ${borderColor}`, borderRadius: '4px', backgroundColor: inputBg, color: textColor, margin: 0, fontSize: 'inherit' }} 
                />
            ...
            <ul style={{ listStyle: 'none', padding: 0, marginTop: '20px', maxHeight: '300px', overflowY: 'auto' }}>
              {racas.map(r => (
                <li key={r.id_raca} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px', borderBottom: `1px solid ${borderColor}` }}>
                  <span style={{ color: textSecundario, fontWeight: 'bold' }}>{r.nome_raca} <small style={{ color: isEscuro ? '#888' : '#777' }}>({r.nome_especie})</small></span>
