...
export default function BuscarAnimal() {
  ...
  const salvarEdicao = async (e) => {
    e.preventDefault();
    try {
      const res = await fetch(`http://localhost:3000/editar-animal/${formDados.id_animal}`, {
        method: 'PUT',
  ...
  return (
    <div style={styles.body}>
      <div style={styles.container}>
        ...
        <h2 style={styles.h2}>Buscar e Editar Pacientes</h2>
        ...
        <div>
          {animais.length === 0 ? <p>Nenhum paciente encontrado.</p> : animais.map(animal => (
            <div key={animal.id_animal} style={styles.card}>
              <div style={{ flex: 1 }}>
                <h3 style={{ margin: '0 0 10px 0', color: '#0056b3' }}>🐾 {animal.nome || animal.nome_animal}</h3>
                <p style={{ margin: '5px 0' }}><strong>Tutor:</strong> {animal.nome_tutor} (CPF: {animal.cpf})</p>
                <p style={{ margin: '5px 0' }}><strong>Espécie:</strong> {animal.especie}</p>
                <p style={{ margin: '5px 0' }}><strong>Raça:</strong> {animal.raca || 'Não informada'}</p>
                <p style={{ margin: '5px 0' }}><strong>Nascimento:</strong> {animal.data_nascimento ? new Date(animal.data_nascimento).toLocaleDateString('pt-BR') : '-'}</p>
              </div>
      ...
      {modalEdicaoOpen && (
        <div style={styles.modalOverlay}>
          <div style={styles.modalContent}>
            <h3 style={{ marginTop: 0 }}>Editar Paciente</h3>
            <form onSubmit={salvarEdicao}>
              ...
              <div style={{ display: 'flex', gap: '10px', marginTop: '15px' }}>
                <button type="submit" style={{ ...styles.btnAcao, flex: 1, margin: 0 }}>Salvar Alterações</button>
                <button type="button" onClick={() => setModalEdicaoOpen(false)} style={{ ...styles.btnVoltar, flex: 1, margin: 0 }}>Cancelar</button>
              </div>
            </form>
          </div>
        </div>
