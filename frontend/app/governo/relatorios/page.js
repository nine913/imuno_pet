    ...
    <LayoutPainel>
      <style>{`
        ...
        @media print {
          ...
          .total-box { border: 2px solid #ea580c; color: black !important; background-color: white !important; }
      ...
      <div style={{ padding: '40px', maxWidth: '1200px', margin: '0 auto', color: textColor, fontFamily: '"Inter", sans-serif' }}>
        ...
        <div className="nao-imprimir" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '32px', flexWrap: 'wrap', gap: '15px' }}>
          <div>
            <h2 style={{ margin: '0 0 8px 0', color: headerColor, fontSize: '28px', fontWeight: '800', letterSpacing: '-0.5px' }}>Relatório Governamental</h2>
            <p style={{ margin: 0, color: textSecundario, fontSize: '15px' }}>Dados consolidados de vacinação e saúde animal na região.</p>
          </div>
          <div style={{ display: 'flex', gap: '12px' }}>
            ...
            <button className="premium-btn" style={{ padding: '12px 20px', color: 'white', border: 'none', borderRadius: '10px', cursor: 'pointer', backgroundColor: '#ea580c', fontWeight: '600', fontSize: '14px', display: 'flex', alignItems: 'center', gap: '8px', boxShadow: '0 4px 6px -1px rgba(234, 88, 12, 0.2)' }} onClick={baixarPDF}>
        ...
        <div id="area-relatorio" style={{ backgroundColor: bgCard, padding: '32px', borderRadius: '16px', border: `1px solid ${borderColor}`, boxShadow: '0 10px 15px -3px rgba(0, 0, 0, 0.05)' }}>
          <h2 className="nao-imprimir" style={{ color: headerColor, marginTop: 0, marginBottom: '24px', fontSize: '20px', display: 'none' }}>Relatório Governamental de Vacinação Animal</h2>
          ...
          <div className="nao-imprimir" style={{ display: 'flex', gap: '16px', backgroundColor: inputBg, padding: '24px', borderRadius: '12px', marginBottom: '24px', alignItems: 'flex-end', flexWrap: 'wrap', border: `1px solid ${borderColor}` }} data-html2canvas-ignore="true">
            ...
            <button className="premium-btn" style={{ padding: '0 24px', color: 'white', border: 'none', borderRadius: '10px', cursor: 'pointer', backgroundColor: '#ea580c', fontWeight: '600', height: '45px', fontSize: '14px' }} onClick={() => gerarRelatorio()}>
              Gerar Relatório
            </button>
          </div>

          <div className="total-box" style={{ backgroundColor: isEscuro ? '#7c2d12' : '#ffedd5', padding: '16px', borderRadius: '12px', textAlign: 'center', marginBottom: '24px', border: `1px solid ${isEscuro ? '#9a3412' : '#fed7aa'}` }}>
            <h2 style={{ margin: 0, color: isEscuro ? '#fed7aa' : '#9a3412', fontSize: '18px', fontWeight: '700' }}>{dadosRelatorio.length} Ocorrências Registradas</h2>
          </div>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '14px' }}>
              <thead>
                <tr>
                  ...
                  <th style={{ borderBottom: `2px solid ${borderColor}`, padding: '16px', textAlign: 'left', backgroundColor: isEscuro ? '#0f172a' : '#f8fafc', color: textSecundario, fontWeight: '700' }}>Raça</th>
                  ...
                  <th style={{ borderBottom: `2px solid ${borderColor}`, padding: '16px', textAlign: 'left', backgroundColor: isEscuro ? '#0f172a' : '#f8fafc', color: textSecundario, fontWeight: '700' }}>Contato</th>
                  <th style={{ borderBottom: `2px solid ${borderColor}`, padding: '16px', textAlign: 'left', backgroundColor: isEscuro ? '#0f172a' : '#f8fafc', color: textSecundario, fontWeight: '700' }}>Localidade</th>
                </tr>
              </thead>
              <tbody>
                {carregando ? (
                  ...
                  dadosRelatorio.map((item, idx) => {
                    ...
                    return (
                      <tr key={idx} style={{ backgroundColor: bgColor, color: textColor, borderBottom: `1px solid ${borderColor}` }}>
                        ...
                        <td style={{ padding: '16px', textAlign: 'left', fontWeight: '600' }}>{item.nome_animal}</td>
                        <td style={{ padding: '16px', textAlign: 'left', color: textSecundario }}>{item.especie}</td>
                        <td style={{ padding: '16px', textAlign: 'left', color: textSecundario }}>{item.raca || '-'}</td>
                        <td style={{ padding: '16px', textAlign: 'left' }}>{item.nome_tutor}</td>
                        <td style={{ padding: '16px', textAlign: 'left' }}>{item.telefone}</td>
                        <td style={{ padding: '16px', textAlign: 'left', color: textSecundario }}>{item.bairro}, {item.cidade}</td>
                      </tr>
                    );
                  })
                )}
