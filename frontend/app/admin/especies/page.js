    ...
    <LayoutPainel>
      ...
      <div style={{ padding: '40px', maxWidth: '1000px', margin: '0 auto', color: textColor, fontFamily: '"Inter", sans-serif' }}>
        ...
        <div style={{ marginBottom: '32px' }}>
          <h2 style={{ color: headerColor, marginTop: 0, marginBottom: '8px', fontSize: '28px', fontWeight: '800', letterSpacing: '-0.5px' }}>Catálogo Base de Espécies</h2>
          <p style={{ color: textSecundario, fontSize: '15px', margin: 0 }}>Gerencie as espécies e raças que ficarão disponíveis para todo o sistema.</p>
        </div>
            ...
            <form onSubmit={cadastrarRaca} style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              ...
              <div style={{ display: 'flex', gap: '12px' }}>
                <input 
                  ...
                  required 
                  style={{ flex: 1, padding: '12px 16px', border: `1px solid ${borderColor}`, borderRadius: '10px', backgroundColor: inputBg, color: textColor, margin: 0, fontSize: '14px', outline: 'none', transition: 'all 0.2s' }} 
                />
                <button type="submit" className="premium-btn" style={{ backgroundColor: '#10b981', color: 'white', border: 'none', padding: '0 20px', borderRadius: '10px', cursor: 'pointer', fontWeight: '600', margin: 0, fontSize: '14px' }}>Salvar</button>
              </div>
            </form>
      ...
      {modalExclusaoOpen && (
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', background: 'rgba(15, 23, 42, 0.7)', backdropFilter: 'blur(4px)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 1000 }}>
          <div style={{ background: bgCard, padding: '32px', borderRadius: '16px', width: '90%', maxWidth: '400px', textAlign: 'center', boxShadow: '0 20px 25px -5px rgba(0,0,0,0.1)', border: altoContraste ? '2px solid #dc2626' : `1px solid ${borderColor}`, color: textColor, boxSizing: 'border-box' }}>
            ...
            <h3 style={{ color: isEscuro ? '#f87171' : '#dc2626', marginTop: 0, fontSize: '20px', fontWeight: '700', marginBottom: '8px' }}>Excluir Registro</h3>
