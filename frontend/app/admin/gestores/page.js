    ...
    <LayoutPainel>
      ...
      <div style={{ padding: '40px', maxWidth: '1000px', margin: '0 auto', color: textColor, fontFamily: '"Inter", sans-serif' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '32px', flexWrap: 'wrap', gap: '15px' }}>
          <div>
            <h2 style={{ margin: '0 0 8px 0', color: headerColor, fontSize: '28px', fontWeight: '800', letterSpacing: '-0.5px' }}>Gestores de Clínica</h2>
            <p style={{ margin: 0, color: textSecundario, fontSize: '15px' }}>Gerencie os usuários administradores das unidades de saúde.</p>
          </div>
          <button className="premium-btn" style={{ backgroundColor: '#10b981', color: 'white', border: 'none', padding: '12px 24px', borderRadius: '10px', cursor: 'pointer', fontWeight: '600', fontSize: '15px', display: 'flex', alignItems: 'center', gap: '8px' }} onClick={abrirModalCadastro}>
            <span style={{ fontSize: '18px' }}>+</span> Novo Gestor
          </button>
        </div>
        ...
        <div style={{ display: 'flex', gap: '12px', marginBottom: '32px' }}>
          <input 
            ...
            onKeyDown={(e) => e.key === 'Enter' && buscarGestores(termoBusca)} 
            placeholder="Pesquisar por nome ou e-mail do gestor..." 
          ...
          />
          <button className="premium-btn" style={{ backgroundColor: '#2563eb', color: 'white', border: 'none', padding: '0 24px', borderRadius: '10px', cursor: 'pointer', fontWeight: '600', flex: 1, margin: 0, fontSize: '15px' }} onClick={() => buscarGestores(termoBusca)}>Pesquisar</button>
        </div>
        ...
        <div>
          {gestores.length === 0 ? (
            ...
            <div style={{ display: 'grid', gap: '20px' }}>
              {gestores.map(gestor => (
                <div key={gestor.id_gestor} className="premium-card" style={{ border: `1px solid ${borderColor}`, padding: '24px', borderRadius: '16px', backgroundColor: bgCard, display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '20px', boxShadow: '0 4px 6px -1px rgba(0, 0, 0, 0.05)' }}>
                  <div style={{ flex: 1 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '12px' }}>
                      ...
                      <h3 style={{ margin: 0, color: headerColor, fontSize: '20px', fontWeight: '700' }}>{gestor.nome_completo}</h3>
                    </div>
                    ...
                    <div style={{ backgroundColor: inputBg, padding: '16px', borderRadius: '12px', border: `1px solid ${borderColor}`, display: 'flex', flexDirection: 'column', gap: '8px' }}>
                      <div>
                        <span style={{ display: 'inline-block', fontSize: '12px', color: textSecundario, textTransform: 'uppercase', fontWeight: '700', width: '70px' }}>E-mail:</span>
                        <span style={{ fontSize: '14px', color: textColor, fontWeight: '500' }}>{gestor.email}</span>
                      </div>
                      <div>
                        <span style={{ display: 'inline-block', fontSize: '12px', color: textSecundario, textTransform: 'uppercase', fontWeight: '700', width: '70px' }}>Clínica:</span>
                        <span style={{ fontSize: '14px', color: textColor, fontWeight: '500', backgroundColor: isEscuro ? '#064e3b' : '#ecfdf5', color: isEscuro ? '#34d399' : '#047857', padding: '2px 8px', borderRadius: '4px' }}>{gestor.nome_fantasia}</span>
                      </div>
                    </div>
                  </div>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', minWidth: '140px' }}>
                    <button className="premium-btn" style={{ color: '#0f172a', padding: '10px 16px', border: 'none', borderRadius: '10px', cursor: 'pointer', fontWeight: '600', backgroundColor: '#f59e0b', fontSize: '14px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }} onClick={() => abrirModalEdicao(gestor)}>
                      <span style={{ filter: sombraEmoji }}>✏️</span> Editar
                    </button>
                    <button className="premium-btn" style={{ color: 'white', padding: '10px 16px', border: 'none', borderRadius: '10px', cursor: 'pointer', fontWeight: '600', backgroundColor: '#ef4444', fontSize: '14px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }} onClick={() => { setIdParaExcluir(gestor.id_gestor); setModalExclusaoOpen(true); }}>
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', background: 'rgba(15, 23, 42, 0.7)', backdropFilter: 'blur(4px)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 1000 }}>
          <div style={{ backgroundColor: bgCard, padding: '32px', borderRadius: '16px', width: '90%', maxWidth: '450px', boxShadow: '0 20px 25px -5px rgba(0,0,0,0.1)', color: textColor, border: altoContraste ? '2px solid #ffcc00' : `1px solid ${borderColor}`, maxHeight: '90vh', overflowY: 'auto', boxSizing: 'border-box' }}>
            <h3 style={{ color: headerColor, marginTop: 0, marginBottom: '24px', fontSize: '20px', fontWeight: '700', display: 'flex', alignItems: 'center', gap: '8px' }}>
              {isEdicao ? <><span style={{ filter: sombraEmoji }}>✏️</span> Editar Gestor</> : <><span style={{ filter: sombraEmoji }}>👔</span> Cadastrar Gestor</>}
            </h3>
            ...
            <form onSubmit={submitForm} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              ...
              {!isEdicao && (
                <div>
                  <label style={{ display: 'block', fontWeight: '600', marginBottom: '6px', color: textSecundario, fontSize: '13px' }}>Senha (Criação de Conta):</label>
              ...
              <div>
                <label style={{ display: 'block', fontWeight: '600', marginBottom: '6px', color: textSecundario, fontSize: '13px' }}>Vincular à Clínica:</label>
                <select className="premium-input" value={formDados.id_clinica} onChange={e => setFormDados({...formDados, id_clinica: e.target.value})} required style={{ width: '100%', padding: '12px 16px', border: `1px solid ${borderColor}`, borderRadius: '10px', boxSizing: 'border-box', backgroundColor: inputBg, color: textColor, fontSize: '14px', outline: 'none', transition: 'all 0.2s' }}>
              ...
              <div style={{ display: 'flex', gap: '12px', marginTop: '16px' }}>
                ...
                <button type="button" className="premium-btn" style={{ color: 'white', padding: '14px', border: 'none', borderRadius: '10px', cursor: 'pointer', fontWeight: '600', backgroundColor: '#64748b', flex: 1, fontSize: '15px' }} onClick={fecharModais}>Cancelar</button>
              </div>
            </form>
            {mensagemForm.texto && (
      ...
      {modalExclusaoOpen && (
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', background: 'rgba(15, 23, 42, 0.7)', backdropFilter: 'blur(4px)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 1000 }}>
          <div style={{ background: bgCard, padding: '32px', borderRadius: '16px', textAlign: 'center', width: '90%', maxWidth: '400px', boxShadow: '0 20px 25px -5px rgba(0,0,0,0.1)', color: textColor, border: altoContraste ? '2px solid #dc2626' : `1px solid ${borderColor}`, boxSizing: 'border-box' }}>
            ...
            <p style={{ color: textColor, fontSize: '15px', marginBottom: '24px', lineHeight: '1.5' }}>Confirma a exclusão deste gestor e do seu acesso ao sistema?</p>
