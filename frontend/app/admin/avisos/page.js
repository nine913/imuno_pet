...
export default function AdminAvisos() {
  ...
  const deletarAviso = async (id) => {
    ...
    try {
      const res = await fetch(`http://localhost:3000/admin/deletar-aviso/${id}`, { method: 'DELETE' });
      if (res.ok) buscarAvisos();
    } catch (error) {
  ...
  return (
    <div style={styles.body}>
      <div style={styles.container}>
        ...
        <div>
          {avisos.length === 0 ? <p>Nenhum aviso criado.</p> : avisos.map(aviso => (
            <div key={aviso.id_aviso} style={{ ...styles.card, borderLeft: `5px solid ${aviso.tipo === 'URGENTE' ? '#dc3545' : aviso.tipo === 'ALERTA' ? '#ffc107' : '#17a2b8'}` }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '5px' }}>
                ...
                <button style={{ ...styles.btnAcao, backgroundColor: '#dc3545' }} onClick={() => deletarAviso(aviso.id_aviso)}>🗑️ Excluir</button>
              </div>
            </div>
          ))}
        </div>
      ...
      {modalFormOpen && (
        <div style={styles.modalOverlay}>
          <div style={styles.modalContent}>
            <h3 style={{ marginTop: 0 }}>{isEdicao ? '✏️ Editar Aviso' : 'Criar Novo Aviso'}</h3>
            <form onSubmit={submitForm}>
              ...
              <div style={{ display: 'flex', gap: '10px', marginTop: '15px' }}>
                <button type="submit" style={{ ...styles.btnAcao, backgroundColor: '#28a745', width: '100%' }}>Salvar Aviso</button>
                <button type="button" onClick={() => setModalFormOpen(false)} style={{ ...styles.btnVoltar, margin: 0, width: '100%' }}>Cancelar</button>
              </div>
            </form>
...
const styles = {
  ...
  btnAcao: { color: 'white', padding: '10px 15px', border: 'none', borderRadius: '4px', cursor: 'pointer', fontSize: '14px' },
