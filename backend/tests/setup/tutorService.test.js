...
describe('deletarTutor', () => {
    ...
    test('deve impedir exclusão quando houver animais vinculados', async () => {
...
// Casos: deletarTutor
// ----------------------------------------------------------------------------
// 2) Deve retornar erro quando tutor não existir

test('deve retornar erro quando tutor não existir', async () => {

    // 1) Mock da validação: não há animais vinculados (total = 0)
    // 2) Mock do tutor buscado: retorno vazio => tutor inexistente
    db.query
...
// Casos: deletarTutor
// -----------------------------------------------------------------------------
// 3) Deve excluir tutor e usuário com sucesso

test('deve excluir tutor e usuário com sucesso', async () => {

    // Fluxo esperado:
    // - Passo A: validação de animais vinculados (total = 0)
    // - Passo B: busca do id_usuario associado ao tutor
...
// - Impedir cadastro com CPF já existente
// - Cadastro completo (usuário + tutor + animal)
// ============================================================================

test('deve impedir cadastro com email já existente', async () => {
...
test('deve impedir cadastro com CPF já existente', async () => {
...
test('deve cadastrar tutor e animal com sucesso', async () => {
...
test('deve retornar animais do tutor', async () => {
    ...
    const resultado =
        await tutorService.getTutorAnimais(1);
...
test('deve retornar lista vazia quando não houver alertas', async () => {
    ...
    const resultado =
        await tutorService.getTutorAlertas(1);
...
test('deve retornar alertas do tutor', async () => {
